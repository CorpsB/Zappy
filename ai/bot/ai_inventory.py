...
def parse_inventory(inv_string, thread_name="UnknownThread"):
    ...
    if not inv_string or not inv_string.startswith('[') or not inv_string.endswith(']'):
        ap.safe_print(f"[{thread_name}] CRITICAL PARSE ERROR: Received non-inventory: '{inv_string}'")
        at.trigger_exit(f"{thread_name} InvParseFormat")
        return None
    try:
        ...
        ap.safe_print(f"[{thread_name}] CRITICAL PARSE ERROR: Inventory ValueError for '{inv_string}': {e}")
        at.trigger_exit(f"{thread_name} InvParseValueError")
        return None
    except Exception as e:
        ap.safe_print(f"[{thread_name}] CRITICAL PARSE ERROR: Inventory generic Exception for '{inv_string}': {e}")
        at.trigger_exit(f"{thread_name} InvParseException")
        return None

def get_inventory_and_food(sock, reader, thread_name):
    """Fetches and parses inventory, returns inventory dict and food level."""

    inv_str = aso.do_action(sock, reader, "Inventory", thread_name)
    if inv_str == "dead":
        return None, 0
